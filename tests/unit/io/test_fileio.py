...
def test_is_root_when_true():
    ...
    assert coalescenceml.io.utils.is_root("/")


def test_is_root_when_false(tmp_path):
    """Check is_root returns false if path isn't the root"""
    assert coalescenceml.io.utils.is_root(tmp_path) is False
...
def test_find_files_returns_generator_object_when_file_present(tmp_path):
    ...
    with open(temp_file, "w"):
        assert isinstance(
            coalescenceml.io.utils.find_files(
...
def test_find_files_when_file_absent(tmp_path):
    ...
    with open(temp_file, "w"):
        with pytest.raises(StopIteration):
            assert next(coalescenceml.io.utils.find_files(str(tmp_path), "abc*.*"))
...
def test_is_remote_when_using_remote_prefix(filesystem):
    ...
    assert coalescenceml.io.utils.is_remote(some_random_path)
...
def test_is_remote_when_using_non_remote_prefix(filesystem):
    ...
    a remote prefix"""
    some_random_path = os.path.join(filesystem + "some_directory")
    assert coalescenceml.io.utils.is_remote(some_random_path) is False
...
def test_listdir_returns_one_result_for_one_file(tmp_path):
    ...
    one file created"""
    coalescenceml.io.utils.create_file_if_not_exists(
...
def test_remove_function(tmp_path) -> None:
    ...
    coalescenceml.io.utils.create_file_if_not_exists(new_file_path)
...
def test_create_file_if_not_exists_does_not_overwrite(tmp_path) -> None:
    ...
    coalescenceml.io.utils.create_file_if_not_exists(temporary_file)
...
def test_create_dir_if_not_exists(tmp_path) -> None:
    ...
    coalescenceml.io.utils.create_dir_if_not_exists(os.path.join(tmp_path, "new_dir"))
...
def test_create_dir_recursive_if_not_exists(tmp_path) -> None:
    ...
    coalescenceml.io.utils.create_dir_recursive_if_not_exists(
...
def test_resolve_relative_path(tmp_path) -> None:
    ...
    assert current_working_directory == coalescenceml.io.utils.resolve_relative_path(
        "."
    )
...
def test_copy_dir_copies_dir_from_source_to_destination(tmp_path) -> None:
    ...
    coalescenceml.io.utils.copy_dir(
...
def test_get_grandparent_gets_the_grandparent_directory(tmp_path) -> None:
    ...
    grandparent = coalescenceml.io.utils.get_grandparent(
        os.path.join(tmp_path, "new_dir/new_dir2")
    )
...
def test_get_parent_gets_the_parent_directory(tmp_path) -> None:
    ...
    parent = coalescenceml.io.utils.get_parent(
        os.path.join(tmp_path, "new_dir/new_dir2")
    )
...
def test_convert_to_str_converts_to_string(tmp_path) -> None:
    ...
    assert isinstance(
        coalescenceml.io.utils.convert_to_str(bytes(str(tmp_path), "ascii")), str
    )
    assert coalescenceml.io.utils.convert_to_str(bytes(str(tmp_path), "ascii")) == str(
        tmp_path
    )